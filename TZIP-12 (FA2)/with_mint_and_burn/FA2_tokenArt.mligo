...
module Constants = struct
   let mintPrice = 50000mutez (* TODO: check amount *)
   let TAContract : address = "K1..."
   let withdrawalWallet : address = "K1..."
   let signature : bytes = "..."
end
...
module Errors = struct
   ...
   let not_enough_tez  = "TA_NOT_ENOUGH_TEZ_FOR_TRANSACTION"
end
...
module TokenMetadata = struct
   ...
   *)
   ...
   type t = {token_info : (nat, data) big_map; total : nat  }
...
[@view] let total_supply ((), s) : (unit * storage)) : nat =
...
let _transferToWallet () =
   let amount_ = Tezos.amount in
   assert_with_error (amount_ >= Constants.mintPrice) Errors.not_enough_tez;
   (* TODO : check for parameters *)
   Tezos.transaction () amount_ Constants.withdrawalWallet

let tokenArtMint ({tokenURI;license;version;signature} : token_art_mint) (s : storage) =
   let () = Storage.assert_admin s in  (* only owner guard *)
   let op1 = _transferToWallet () in
   let token_id = totalSupply () in
   let md = Storage.get_token_metadata s in
   let md = TokenMetadata.add_new_token md token_id tokenURI in
   let s = Storage.set_token_metadata s md in
   let sender = Tezos.sender in
   let ledger = Storage.get_ledger s in
   (* Check, sender is the address that directly calls FA2, is that what we want for the owner ?*)
   let ledger = Ledger.mint_token ledger token_id sender in
   let s = Storage.set_ledger s ledger in
   let tokenArtContract = match Tezos.get_entrypoint_opt "addLicenseForTokenFromContract" with
      Some c -> c | None -> failwith Errors.token_art_not_found
   (* TODO : check for actual layout with the archetype contract *)
   type tokenArtParams = {
      fa2      : address;
      token_id : nat;
      license  : nat;
   ...
   let op2 = Tezos.transaction {fa2 = Tezos.self_adress; token_id;license;version;sender;signature} 0tez tokenArtContract
   ([op1; op2]: operation list),s
...
type parameter = [@layout:comb]
   ...
   | TokenArtMint of token_art_mint
...
| TokenArtMint      p -> tokenArtMint p s
