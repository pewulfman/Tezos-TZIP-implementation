#import "../with_mint_and_burn/FA2_single_asset.mligo" "FA2_single_asset"
...
let test_set_admin_check_sender_wrong_sender =
  ...
  match result with
    Success _gas -> failwith "This test should fail"
  | Fail (Rejected (err, _))  -> assert (Test.michelson_equal err (Test.eval FA2_single_asset.Errors.requires_admin))
  | Fail _ -> failwith "invalid test failure"
...
let test_mint_correct = 
  ...
  let contr = Test.to_contract t_addr in
  let () = Test.set_source owner1 in
  let _ = Test.transfer_to_contract_exn contr (Mint_token [{owner = owner1; amount_ = 10n} ]) 0tez in
  ()
...
let test_mint_wrong_sender = 
  ...
  let contr = Test.to_contract t_addr in
  let () = Test.set_source owner2 in
  let result = Test.transfer_to_contract contr (Mint_token [{owner = owner1; amount_ = 10n} ]) 0tez in
  match result with
    Success _gas -> failwith "This test should fail"
  | Fail (Rejected (err, _))  -> assert (Test.michelson_equal err (Test.eval FA2_single_asset.Errors.requires_admin))
  | Fail _ -> failwith "invalid test failure"
...
let test_burn_token_correct = 
  ...
  let () = Test.set_source owner1 in
  let _ = Test.transfer_to_contract_exn contr (Mint_token [{owner = owner1; amount_ = 10n}]) 0tez in
  let _ = Test.transfer_to_contract_exn contr (Burn_token [{owner = owner1; amount_ = 10n}]) 0tez in
  ()
...
let test_burn_token_wrong_too_much = 
  ...
  let () = Test.set_source owner1 in
  let _ = Test.transfer_to_contract_exn contr (Mint_token [{owner = owner1; amount_ = 10n}]) 0tez in
  let result = Test.transfer_to_contract contr (Burn_token [{owner = owner1; amount_ = 200n}]) 0tez in
  match result with
    Success _gas -> failwith "This test should fail"
  | Fail (Rejected (err, _))  -> assert (Test.michelson_equal err (Test.eval FA2_single_asset.Errors.ins_balance))
  | Fail _ -> failwith "invalid test failure"
...
let test_burn_token_wrong_sender = 
  ...
  let _ = Test.transfer_to_contract_exn contr (Mint_token [{owner = owner1; amount_ = 10n}]) 0tez in
  let () = Test.set_source owner2 in
  let result = Test.transfer_to_contract contr (Burn_token [{owner = owner1; amount_ = 10n}]) 0tez in
  match result with
    Success _gas -> failwith "This test should fail"
  | Fail (Rejected (err, _))  -> assert (Test.michelson_equal err (Test.eval FA2_single_asset.Errors.requires_admin))
  | Fail _ -> failwith "invalid test failure"
